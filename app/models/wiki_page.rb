...
#                :description => :text,
#                :datetime => :created_on,
#                :url => Proc.new {|o| {:controller => 'wiki', :id => o.wiki.group_id, :page => o.title}}
#
#  acts_as_searchable :columns => ['title', 'text'],
#                     :include => [{:wiki => :group}, :content],
#                     :group_key => "#{Wiki.table_name}.group_id"
#
  ...
  def editable_by?(usr)
    #!protected? || usr.allowed_to?(:protect_wiki_pages, wiki.group)
    !protected? || usr.is_member_of?(wiki.group)
  end
  ...
  def validate
    ...
    errors.add(:parent_title, :not_same_group) if parent && (parent.wiki_id != wiki_id)
  end
end
